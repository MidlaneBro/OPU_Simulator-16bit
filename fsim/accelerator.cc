...
void Device::Activation(std::vector<acc_type>& data, int type) {
  if (type == NO_RELU_CODE || reg_.activation == 0) {
    ...
    for (auto& item : data) {
      if (item < 0) {
        if (item & 0x4) { // TODO: change this hex value - depends on roundign vs truncation? might be to add 2 more 0'x to 0x400
          item = (item >> 3) + 1;
        } else {
          item = item >> 3;
        }
      ...
      for (int p = 0; p < compute->ker_round; p++) {
        ...
        if (ipa_.GetOutputNum() >= compute->output_channel) {
          ...
          if (compute->add_bias) {
...
#ifdef OPU16
            ipa_.adder_b.push_back(
                 Saturate16(value << compute->shift_num_bias, value > 0, ACC_SIZE));
            }
#endif
            ...
            std::vector<acc_type> tmp = ipa_.adder_buf_b_.AsVec(0, 64/(PRECISION/8), PRECISION*2);
            for (auto value : tmp) {
...
void Device::RunComputeFC(OPUComputeInsn* compute) {
  ...
  std::vector<acc_type> bias = ipa_.adder_buf_b_.AsVec(0, 64, PRECISION*2);
    ...
    if (ipa_.GetOutputNum() >= compute->output_channel) {
      ...
      if (compute->add_bias) {
        ...
        ipa_.adder_buf_b_.Load<1024*(PRECISION/8)>(MemOp(0, 1, 0),
            tmp_buf_.BeginPtr(tmp_addr));
        std::vector<acc_type> tmp = ipa_.adder_buf_b_.AsVec(0, 64, PRECISION*2);
        for (auto value : tmp) {
          ipa_.adder_b.push_back(static_cast<acc_type>(value) << 10);
        }
      }
        ...
        if (ipa_.GetOutputNum() >= compute->output_channel) {
          ...
          if (compute->add_bias) {
            ...
            ipa_dw_.adder_buf_b_.Load<128*PRECISION>(MemOp(0, 1, 0),
                tmp_buf_.BeginPtr(tmp_addr));
            std::vector<acc_type> tmp = ipa_dw_.adder_buf_b_.AsVec(0, 64, PRECISION*2);
            for (auto value : tmp) {
