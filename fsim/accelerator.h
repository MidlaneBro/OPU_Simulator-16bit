...
class IPA {
 public:
  ...
  PE_buf_t wgt_buf_a_;
  PE_buf_t wgt_buf_b_;
  using Acc_buf_t = SRAM<ACC_STORAGE*(64/(PRECISION/8)), 1, 1>; // 64 32-bit adders for 8-bit, 32 64-bit adders for 16-bit
  ...
  void Accumulate(int fm_lshift_num, bool cut, void* dst,
    ...
    std::vector<acc_type> adder_a(64/(PRECISION/8) - psum.size(), 0);
...
#ifdef OPU16
      adder_a.push_back(
        Saturate16(value << fm_lshift_num, value > 0, ACC_SIZE));
#endif
    }
    ...
    for (int ii = 0; ii < 64/(PRECISION/8); ii++) {
      acc_type p = *(ia++) + *(ib++);
      res.push_back(p);
    }
    // ACC_TYPE--cut->2*PRECISION (eg: for 8 bit we cut from 27b -> 16, for 16 bit we cut from 42b -> 32)
    for (auto &item : res) {
#ifdef OPU16
	item = item = Saturate16(item >> 10, item > 0, 2*PRECISION);
#endif
#ifdef OPU8
    ...
    std::memcpy(dst, &trunc[0], 64*(PRECISION*2)/8);  // byte count
  }
};
