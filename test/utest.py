...
#REF_FILE = "fm_output.txt"
REF_FILE = "store_output.txt"
...
def test_tiny_yolo():
    ...
    with open('config.h', 'w') as fw:
        print("#define "+DEBUG_DEFINE, file=fw)
    ...
    run_shell_cmd([sim_x])
    # check
    run_shell_cmd(['cp', os.path.join(cwd, '../example', test_name, REF_FILE), './gt.txt'])
    def hex2int(x):
        bstr = bin(int(x, 16))[2:]
        if len(bstr)<26:
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(dat1)):
            if not dat[j]==dat1[j]:
                print("dat[j] is ", hex(dat[j]))
                print("dat1[j] is ", hex(dat1[j]))
